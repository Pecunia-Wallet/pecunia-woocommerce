...
final class WC_Gateway_Pecunia extends WC_Payment_Gateway
{
    ...
    public function poll_pending_invoices(): void
    {
        ...
        try {
            $orderIds = wc_get_orders( array(
                ...
                'meta_query'   => array(
                    array(
                        'key'     => Pecunia_Order::META_INVOICE_STATUS,
    ...
    private function render_payment_page( $order_id ): void
    {
        ...
        // Escape before output (template should also escape, but we ensure safety here)
        $paymentUrl = esc_url( $paymentUrl );
        $invoiceId  = esc_html( $invoiceId );
    ...
    public function handle_callback(): void
    {
        ...
        $invoiceId = sanitize_text_field( (string) $invoiceId );
        ...
        if ( ! $storedInvoice || ( $invoiceId !== '' && $storedInvoice['invoice_id'] !== '' && $storedInvoice['invoice_id'] !== $invoiceId ) ) {
            $metaInvoiceId = (string) $order->get_meta( Pecunia_Order::META_INVOICE_ID, true );
    ...
    private function createInvoiceForOrder( WC_Order $order, string $fingerprint ): array
    {
        ...
        $created = array(
            'invoice_id'   => sanitize_text_field( $ref->id ),
            'href'         => esc_url_raw( $ref->href ),
            'payment_url'  => esc_url_raw( $ref->paymentUrl ),
            'status'       => 'pending',
            'fingerprint'  => $fingerprint,
            'created_at'   => gmdate( 'c' ),
    ...
    private function normalizeInvoice( Invoice $invoice ): array
    {
        ...
        // Sanitize critical fields before they touch the database
        $sanitizedPayload = array(
            'id'                  => sanitize_text_field( $invoice->id ),
            'paymentUrl'          => esc_url_raw( $invoice->paymentUrl ),
            'status'              => $invoice->status, // will be normalized elsewhere
            'amount'              => (array) $invoice->amount,
            'sourceCurrency'      => sanitize_text_field( $invoice->sourceCurrency ),
        ...
        return array(
            ...
            'href'         => '',
            'payment_url'  => esc_url_raw( $invoice->paymentUrl ),
            ...
            'currency'     => sanitize_text_field( $invoice->sourceCurrency ),
            'meta'         => $invoice->meta,
        );
    }
    ...
    private function applyStatusToOrder( WC_Order $order, string $incomingStatus, array $payload, string $invoiceId = '', ?string $currentStatusOverride = null ): void
    {
        ...
        $currentStatus = $currentStatusOverride !== null
            ...
            : $this->normalizeStatus( (string) $order->get_meta( Pecunia_Order::META_INVOICE_STATUS, true ) );
        ...
        if ( $invoiceId !== '' ) {
            $order->update_meta_data( Pecunia_Order::META_INVOICE_ID, $invoiceId );
        }

        if ( isset( $payload['paymentUrl'] ) ) {
            $order->update_meta_data( Pecunia_Order::META_INVOICE_URL, esc_url_raw( (string) $payload['paymentUrl'] ) );
        }
        ...
        if ( in_array( $incomingStatus, array( 'pending', 'staggering' ), true ) ) {
            $order->update_status( 'pending', __( 'Awaiting Pecunia payment.', 'pecunia-wallet-payment-gateway' ) );
            $order->add_order_note( sprintf( __( 'Invoice %1$s is still awaiting payment (%2$s).', 'pecunia-wallet-payment-gateway' ), $invoiceId ?: $order->get_meta( Pecunia_Order::META_INVOICE_ID, true ), $incomingStatus ) );
            return;
        }

        if ( in_array( $incomingStatus, array( 'completed', 'overpaid' ), true ) ) {
            $order->update_meta_data( Pecunia_Order::META_INVOICE_STATUS, $incomingStatus );
            $order->save();
            ...
            $order->add_order_note( $note . ' ' . sprintf( __( 'Invoice ID: %s', 'pecunia-wallet-payment-gateway' ), $invoiceId ?: (string) $order->get_meta( Pecunia_Order::META_INVOICE_ID, true ) ) );
            return;
        }
        ...
        if ( $incomingStatus === 'expired' ) {
            ...
            $order->add_order_note( sprintf( __( 'Invoice %s expired.', 'pecunia-wallet-payment-gateway' ), $invoiceId ?: (string) $order->get_meta( Pecunia_Order::META_INVOICE_ID, true ) ) );
        }
    }
    ...
    private function parseCallbackPayload( string $raw ): array
    {
        ...
        // Last resort: sanitize the entire POST array
        $post = wp_unslash( $_POST );
        if ( ! empty( $post ) ) {
            return map_deep( $post, 'sanitize_text_field' );
        }
    ...
    private function normalizeStatus( string $status ): string
    {
        ...
        return match ( $status ) {
            'awaiting' => 'pending',
            'paid' => 'completed',
            'pending', 'staggering', 'completed', 'overpaid', 'expired' => $status,
            default => 'pending', // unknown statuses fall back to a safe default
        };
    }
