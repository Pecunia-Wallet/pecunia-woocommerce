...
final class WC_Pecunia_Order
{
	...
	{
		$map = array(
			self::META_INVOICE_ID => $data['invoice_id'] ?? '',
			self::META_INVOICE_URL => $data['invoice_url'] ?? '',
			self::META_INVOICE_HREF => $data['href'] ?? '',
			self::META_INVOICE_PAYMENT_URL => $data['payment_url'] ?? ( $data['invoice_url'] ?? '' ),
			self::META_INVOICE_STATUS => strtolower( (string) ( $data['status'] ?? 'pending' ) ),
			self::META_INVOICE_FINGERPRINT => $data['fingerprint'] ?? '',
			self::META_INVOICE_CREATED_AT => $data['created_at'] ?? gmdate( 'c' ),
			self::META_INVOICE_LAST_SYNC_AT => $data['last_sync_at'] ?? gmdate( 'c' ),
			self::META_INVOICE_EXPIRES_AT => $data['expires_at'] ?? '',
			self::META_INVOICE_PAYLOAD => isset( $data['payload'] ) ? wp_json_encode( $data['payload'] ) : '',
			self::META_INVOICE_ERROR => $data['error'] ?? '',
		);
	...
	{
		$order->update_meta_data( self::META_INVOICE_STATUS, strtolower( $status ) );
		$order->update_meta_data( self::META_INVOICE_LAST_SYNC_AT, gmdate( 'c' ) );
		$order->save();
	}

	public function markCallbackHash( WC_Order $order, string $hash ): void
	{
		$order->update_meta_data( self::META_CALLBACK_HASH, $hash );
		$order->save();
	}
